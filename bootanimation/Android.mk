...
ifeq ($(LCM_WIDTH),)
    $(warning LCM_WIDTH is not set, using default value: 1080)
    LCM_WIDTH := 1080
endif
ifeq ($(LCM_HEIGHT),)
    $(warning LCM_HEIGHT is not set, using default value: 1920)
    LCM_HEIGHT := 1920
endif
...
$(TARGET_GENERATED_BOOTANIMATION): $(SOONG_ZIP)
	...
	@rm -rf $(dir $@)
	@mkdir -p $(dir $@)
	$(hide) tar xfp vendor/openwatchproject/bootanimation/bootanimation.tar -C $(INTERMEDIATES)
	$(hide) if [ $(LCM_HEIGHT) -lt $(LCM_WIDTH) ]; then \
	    IMAGEWIDTH=$(LCM_HEIGHT); \
	else \
	    IMAGEWIDTH=$(LCM_WIDTH); \
	fi; \
	...
	cat vendor/openwatchproject/bootanimation/desc.txt >> $(INTERMEDIATES)/desc.txt
